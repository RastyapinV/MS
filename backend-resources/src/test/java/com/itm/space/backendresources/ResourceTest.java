...
public class ResourceTest extends BaseIntegrationTest {
    ...
    @Autowired
    public ResourceTest(UserService userService, Keycloak keycloak) {
    ...
    @SneakyThrows
    void cleanDb() {
        List<UserRepresentation> userRepresentations = keycloak.realm("ITM").users()
                .search("User");
    ...
    @SneakyThrows
    void getUserById() {
        userService.createUser(new UserRequest("User", "user@gmail.com",
                "test", "User", "User"));
        String testUUID = keycloak.realm("ITM").users().search("User").get(0).getId();
        MockHttpServletRequestBuilder mockHttpServletRequestBuilder =
                get("/api/users/" + testUUID);
        mvc.perform(mockHttpServletRequestBuilder).andExpectAll(status().isOk());
